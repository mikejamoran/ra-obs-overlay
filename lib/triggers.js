...
const SOURCE_TYPES = ['channel_points', 'cheer', 'chat_command', 'subscription', 'raid', 'follow', 'achievement', 'kofi'];
const MAX_QUEUE    = 50;
...
function triggerDefaults() {
  return {
    ...
    source: {
      ...
      minPoints: 0,
      // kofi (donation via Ko-fi webhook); matchType is shared with cheer
      minAmount: 0,            // 0 = any amount, in the Ko-fi account's currency
      kofiType: 'any',         // 'any' | 'Donation' | 'Subscription' | 'Shop Order' | 'Commission'
    },
    actions: {
      ...
      zIndex: 999,            // alerts can layer above or below widgets/panels
    },
  };
}
...
function normalizeTrigger(raw, existing) {
  ...
  const t = {
    ...
    actions: {
      ...
      zIndex: (raw.actions && raw.actions.zIndex !== undefined)
                ? (parseInt(raw.actions.zIndex) || 999)
                : (base.actions.zIndex ?? 999),
    },
  };
...
// ── Engine factory ──────────────────────────────────────────────────────────────
// One engine per user: each holds its own alert queue, cooldowns, and deps
// ({ getState, saveConfig, broadcast, checkPermission }).
...
function renderTemplate(tpl, evt) {
  return String(tpl || '')
    ...
    .replace(/\{currency\}/gi, evt.currency ?? '')
...
function handleEvent(evt) {
  ...
  if (evt.type === 'kofi') {
    // Same tier logic as cheers, on the decimal donation amount: an exact-amount
    // trigger wins, otherwise the highest minAmount ≤ amount fires.
    const amount = parseFloat(evt.amount) || 0;
    const eligible = candidates.filter(t =>
      !t.source.kofiType || t.source.kofiType === 'any' || t.source.kofiType === evt.kofiType);
    let chosen = eligible.find(t =>
      t.source.matchType === 'exact' && Math.abs((parseFloat(t.source.minAmount) || 0) - amount) < 0.005);
    if (!chosen) {
      chosen = eligible
        .filter(t => t.source.matchType !== 'exact' && amount >= (parseFloat(t.source.minAmount) || 0))
        .sort((a, b) => (parseFloat(b.source.minAmount) || 0) - (parseFloat(a.source.minAmount) || 0))[0];
    }
...
function makeTestEvent(trigger) {
  ...
  return {
    ...
    user:     'TestUser',
    amount:   trigger.source.type === 'cheer' ? (parseInt(trigger.source.bits) || 100)
            : trigger.source.type === 'kofi'  ? ((parseFloat(trigger.source.minAmount) || 3).toFixed(2))
            : 1,
    currency: 'GBP',
    kofiType: (trigger.source.kofiType && trigger.source.kofiType !== 'any') ? trigger.source.kofiType : 'Donation',
...
module.exports = { createEngine, SOURCE_TYPES, triggerDefaults, normalizeTrigger };
